...
Whisp ships as an unsigned `.zip` on GitHub Releases. This is the simplest
path that doesn't cost $99/year for an Apple Developer ID. If you fork
Whisp and want to publish signed builds, see [Signing & notarization](#signing--notarization)
at the bottom.

## What `scripts/build-release.sh` produces

```
build/Whisp.app                       # ready-to-run bundle
dist/Whisp-<version>.zip              # zipped via `ditto`, for GitHub Releases
```
...
1. Bump the version in `Sources/Whisp/Resources/Info.plist`
   (`CFBundleShortVersionString`).
2. `./scripts/build-release.sh`
3. Test the produced `.app` on a clean Mac (or `tccutil reset` your own).
4. `gh release create v<version> dist/Whisp-<version>.zip --notes-from-tag`
...
> "Whisp" can't be opened because Apple cannot check it for malicious
> software.

The user accepts once with:

1. **Right-click** `Whisp.app` → **Open**.
...
Whisp has a Developer ID, a cask becomes straightforward:

```ruby
cask "whisp" do
  version "0.2.0"
  sha256 "..."
  url "https://github.com/your-org/whisp/releases/download/v#{version}/Whisp-#{version}.zip"
  name "Whisp"
  desc "Open-source dictation for macOS"
  homepage "https://github.com/your-org/whisp"
  app "Whisp.app"
end
```
...
codesign --force --deep --options runtime \
    ...
    --entitlements Sources/Whisp/Resources/Whisp.entitlements \
    build/Whisp.app

# Notarize
ditto -c -k --keepParent build/Whisp.app dist/Whisp.zip
xcrun notarytool submit dist/Whisp.zip \
    --apple-id you@example.com \
    --team-id TEAMID --password "@keychain:notarytool" \
    --wait

# Staple the ticket
xcrun stapler staple build/Whisp.app
```
